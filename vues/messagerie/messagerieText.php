    ...
    <form  method="post" class="containerMessage" action="index.php?uc=textMessage&action=sendTextMessage">
        ...
        ?>
            <select name="destinataire_<?php echo $classe; ?>" id="<?php echo $classe; ?>" class="classe-select">
                ...
                ?>
                        <option value="<?php echo $eleve['id_user']?>"><?php echo $eleve['classe'] . ' / '; echo $eleve['nom'] . ' '; echo $eleve['prenom']; ?></option>
                <?php
                        }
        ...
        <?php 
            if(isset($_SESSION['erreur']) && $_SESSION['erreur'] == 1){
                echo '<p style="color:red;">'.ERROR_MESSAGE_SHORT_MESSAGE.'</p>';
                unset($_SESSION['erreur']);
            }
        ?>
        <textarea name="msg_content" id="" cols="30" rows="50" maxlength="500" required ></textarea>
        <div>
